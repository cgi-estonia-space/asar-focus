...
struct DSD {
    ...
    void SetDSName(std::string name) { SetStr(ds_name, "DS_NAME", name); }

    void SetType(char type) { SetChar(ds_type, "DS_TYPE", type); }

    void SetFileName(std::string name) { SetStr(filename, "FILENAME", name); }

    void SetDSOffset(size_t offset) { SetAd(ds_offset, "DS_OFFSET", offset, "<bytes>"); }

    void SetDSSize(size_t size) { SetAd(ds_size, "DS_SIZE", size, "<bytes>"); }

    void SetNumDSR(size_t num) { SetAl(num_dsr, "NUM_DSR", num); }

    void SetDSRSize(size_t size) { SetAl(dsr_size, "DSR_SIZE", size, "<bytes>"); }
